...
class PaymentsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(elevation: 0,
        ...
        title: Text('Payments', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)), centerTitle: true),
      body: Consumer<PolicyProvider>(builder: (context, policyProvider, _) {
        if (policyProvider.loading) return const Center(child: CircularProgressIndicator());
        final policies = policyProvider.policies;
        if (policies.isEmpty) {
          return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Icon(Icons.payment, size: 60, color: Colors.grey[300]),
            const SizedBox(height: 12),
            Text('No payments found', style: TextStyle(fontSize: 14, color: Colors.grey[500])),
          ]));
        }
        // Sort by start date, most recent first
        final sorted = List<Map<String, dynamic>>.from(policies);
        sorted.sort((a, b) {
          final da = DateTime.tryParse(a['startDate']?.toString() ?? '') ?? DateTime(1900);
          final db = DateTime.tryParse(b['startDate']?.toString() ?? '') ?? DateTime(1900);
          return db.compareTo(da);
        });
        return SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(), padding: EdgeInsets.all(16),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Recent Payments', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
            const SizedBox(height: 16),
            ...sorted.map((p) => Padding(padding: const EdgeInsets.only(bottom: 14), child: _buildPaymentCard(context, p))),
      ...
      bottomNavigationBar: BottomAppBar(shape: const CircularNotchedRectangle(), notchMargin: 4,
        child: SizedBox(height: 44, child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          _nav(Icons.home_outlined, 'Home', false, onTap: () => Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const CustomerDashboardScreen()), (r) => false)),
          _nav(Icons.description_outlined, 'Policies', false),
          const SizedBox(width: 48),
          _nav(Icons.assignment_outlined, 'Claims', false, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MyClaimsScreen()))),
          _nav(Icons.person_outline, 'Profile', false, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CustomerProfileScreen()))),
        ]))),
    );
  }
  ...
  Widget _buildPaymentCard(BuildContext context, Map<String, dynamic> p) {
    ...
    final icon = policyClass.toLowerCase().contains('motor') ? Icons.directions_car_outlined
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
            decoration: BoxDecoration(color: isActive ? Colors.green : Colors.grey, borderRadius: BorderRadius.circular(14)),
            child: Text(isActive ? 'Active' : 'Expired', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: Colors.white))),
        ]),
        ...
        Align(alignment: Alignment.centerRight, child: GestureDetector(
          onTap: () { Clipboard.setData(ClipboardData(text: p['policyId']?.toString() ?? '')); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Policy ID copied'), duration: Duration(seconds: 1))); },
          child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.copy, size: 13, color: AppTheme.accentOrange), SizedBox(width: 4), Text('Copy', style: TextStyle(fontSize: 10, color: AppTheme.accentOrange, fontWeight: FontWeight.w600))]))),
      ]));
  }
  ...
  Widget _row(String l, String v) => Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
    ...
    Text(v, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.black87))]);
