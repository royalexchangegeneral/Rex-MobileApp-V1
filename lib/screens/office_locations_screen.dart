...
class _OfficeLocationsScreenState extends State<OfficeLocationsScreen> {
  ...
  final List<Map<String, dynamic>> _offices = [
    {'name': 'Head Office', 'address': '26E Abdulrahman Okene Close, off Ligali Ayorinde street, VI, Lagos', 'hours': 'Mon-Fri 8AM-4PM', 'phone': '+234 708 0606 100', 'email': 'general@rexinsure.com', 'lat': 6.4281, 'lng': 3.4219},
    {'name': 'Lagos Main Branch', 'address': '41 Ikorodu Road, Jibowu, Lagos', 'hours': 'Mon-Fri 8AM-4PM', 'phone': '08055266886', 'lat': 6.4905, 'lng': 3.3711},
    {'name': 'Abuja', 'address': '21 Park Plaza, Ademola Adetokunbo Crescent, Wuse II, FCT, Abuja', 'hours': 'Mon-Fri 8AM-4PM', 'phone': '08032122649', 'lat': 9.0579, 'lng': 7.4951},
    {'name': 'Port Harcourt', 'address': '2nd Floor, Plot 278, Diobu (Tombia Plaza) New GRA Phase II, Port Harcourt, River State', 'hours': 'Mon-Fri 8AM-4PM', 'phone': '08066746261', 'lat': 4.8156, 'lng': 7.0498},
  ];
  ...
  Future<void> _getLocation() async {
    try {
      ...
      if (!serviceEnabled) {
        setState(() => _currentAddress = 'Location services disabled. Please enable GPS.');
        return;
      }
      ...
      try {
        ...
        if (placemarks.isNotEmpty) {
          ...
          final parts = [p.street, p.subLocality, p.locality, p.administrativeArea, p.postalCode, p.country].where((s) => s != null && s.isNotEmpty).toList();
          setState(() => _currentAddress = parts.join(', '));
        }
      } catch (_) { setState(() => _currentAddress = '${pos.latitude.toStringAsFixed(4)}, ${pos.longitude.toStringAsFixed(4)}'); }
      _mapController.loadHtmlString(_mapHtml());
      setState(() {});
    } catch (e) {
      print('=== LOCATION ERROR: $e ===');
      if (mounted) setState(() => _currentAddress = 'Could not get location: $e');
    }
  }
  int _findClosest(double lat, double lng) {
    int idx = 0; double minD = double.infinity;
  ...
  double _dist(double lat1, double lng1, double lat2, double lng2) {
    final dLat = (lat2 - lat1) * pi / 180; final dLng = (lng2 - lng1) * pi / 180;
    final a = sin(dLat / 2) * sin(dLat / 2) + cos(lat1 * pi / 180) * cos(lat2 * pi / 180) * sin(dLng / 2) * sin(dLng / 2);
    return 6371 * 2 * atan2(sqrt(a), sqrt(1 - a));
  }

  bool _isOpen() { final n = DateTime.now(); return n.weekday >= 1 && n.weekday <= 5 && n.hour >= 8 && n.hour < 16; }
      ...
      body: SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(16),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...List.generate(_offices.length, (i) {
            final o = _offices[i];
            if (_searchQuery.isNotEmpty && !'${o['name']} ${o['address']}'.toLowerCase().contains(_searchQuery)) return const SizedBox.shrink();
            return Padding(padding: const EdgeInsets.only(bottom: 14), child: _buildCard(context, o, isOpen));
          }),
  ...
  Widget _buildCard(BuildContext ctx, Map<String, dynamic> o, bool isOpen) {
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.grey[50], borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey[200]!)),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(o['address'], style: TextStyle(fontSize: 12, color: Colors.grey[700], height: 1.4)),
        const SizedBox(height: 8),
        Text('Hours: ${o['hours']}', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
        const SizedBox(height: 4),
        Text('Phone: ${o['phone']}', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
        if (o['email'] != null) ...[const SizedBox(height: 4), Text('Email: ${o['email']}', style: TextStyle(fontSize: 12, color: Colors.grey[600]))],
        const SizedBox(height: 12),
        Row(children: [
          ElevatedButton.icon(onPressed: () async {
            final addr = Uri.encodeComponent(o['address']);
            final url = Uri.parse('https://www.google.com/maps/dir/?api=1&destination=$addr');
            try { await launchUrl(url, mode: LaunchMode.externalApplication); } catch (_) {}
          }, icon: const Icon(Icons.directions, size: 16), label: const Text('Get Directions', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
            style: ElevatedButton.styleFrom(backgroundColor: AppTheme.accentOrange, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)))),
          const SizedBox(width: 10),
          Container(decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.grey[300]!)),
            child: IconButton(icon: Icon(Icons.phone, size: 18, color: Colors.grey[500]), constraints: const BoxConstraints(minWidth: 40, minHeight: 40), padding: EdgeInsets.zero,
              onPressed: () async {
                final uri = Uri(scheme: 'tel', path: o['phone']);
                try { final l = await launchUrl(uri, mode: LaunchMode.externalApplication); if (!l && ctx.mounted) _showPhone(ctx, o['phone']); } catch (_) { if (ctx.mounted) _showPhone(ctx, o['phone']); }
              })),
        ]),
      ]));
  }
  void _showPhone(BuildContext c, String p) => showDialog(context: c, builder: (x) => AlertDialog(title: const Text('Call Office'), content: Text(p, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600)), actions: [TextButton(onPressed: () => Navigator.pop(x), child: const Text('OK'))]));
  Widget _nav(IconData i, String l, bool s, VoidCallback? o) => InkWell(onTap: o, child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(i, color: s ? AppTheme.primaryNavy : Colors.grey, size: 20), Text(l, style: TextStyle(fontSize: 10, color: s ? AppTheme.primaryNavy : Colors.grey))]));
}
