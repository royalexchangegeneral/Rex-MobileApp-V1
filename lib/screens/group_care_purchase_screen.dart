...
class GroupCarePurchaseScreen extends StatefulWidget {
  ...
  final String price;
  const GroupCarePurchaseScreen({super.key, required this.optionTitle, required this.price});
  @override
  State<GroupCarePurchaseScreen> createState() => _GroupCarePurchaseScreenState();
}
...
class _GroupCarePurchaseScreenState extends State<GroupCarePurchaseScreen> {
  ...
  final List<String> _relationships = const ['Spouse', 'Parent', 'Child', 'Sibling', 'Friend', 'Others'];

  @override
  void dispose() {
    _groupNameController.dispose(); _groupEmailController.dispose(); _groupAddressController.dispose(); _websiteController.dispose();
    _chairNameController.dispose(); _chairPhoneController.dispose(); _chairNinController.dispose();
    _secNameController.dispose(); _secPhoneController.dispose(); _secNinController.dispose();
    _memberNameController.dispose(); _memberDobController.dispose(); _memberOccupationController.dispose();
    _memberPhoneController.dispose(); _memberEmailController.dispose();
    _nokNameController.dispose(); _nokPhoneController.dispose();
    super.dispose();
  }
  ...
  String _formatNaira(double amount) => '₦${amount.toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+\.)'), (m) => '${m[1]},')}';

  Future<void> _verifyNin(TextEditingController ninController, TextEditingController nameController, TextEditingController phoneController, {required bool isChairman}) async {
    ...
    setState(() { if (isChairman) _isVerifyingChairNin = true; else _isVerifyingSecNin = true; });
    try {
      ...
      print('=== NIN VERIFY (${isChairman ? "Chairman" : "Secretary"}): ${response.statusCode} ===');
      print('Body: ${response.body}');
      setState(() { if (isChairman) _isVerifyingChairNin = false; else _isVerifyingSecNin = false; });
      if ((response.statusCode == 200 || response.statusCode == 201)) {
        ...
        if (data['status'] == 'success' && data['data']?['data']?['kyc'] != null && data['data']['data']['kyc']['firstname'] != null && (data['data']['data']['kyc']['firstname']?.toString() ?? '').isNotEmpty) {
          ...
          setState(() {
            nameController.text = '${kyc['firstname'] ?? ''} ${kyc['surname'] ?? ''}'.trim();
            phoneController.text = kyc['telephoneno']?.toString() ?? '';
            if (isChairman) _chairNinStatus = 'verified'; else _secNinStatus = 'verified';
          });
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${isChairman ? "Chairman" : "Secretary"} NIN verified'), backgroundColor: Colors.green));
        } else {
          final kyc = data['data']?['data']?['kyc'];
          final msg = kyc != null ? (kyc['status']?.toString() ?? 'Verification failed') : 'No data found for this NIN';
  ...
  Future<void> _initiatePayment() async {
    final email = _groupEmailController.text.trim().isNotEmpty ? _groupEmailController.text.trim() : 'customer@rexinsure.com';
    ...
    try {
      final result = await PaymentService.initiatePurchase(
        ...
        extraFields: {
          ...
          'members': _coverMembers.map((m) => {'names': m['name'] ?? '', 'gender': m['gender'] ?? '', 'mobileno': m['phone'] ?? '', 'email': m['email'] ?? '', 'dob': m['dob'] ?? '', 'nok': m['nokName'] ?? '', 'nokphone': m['nokPhone'] ?? ''}).toList(),
        },
      );
        ...
        if (res != null && res.success && mounted) {
          showDialog(context: context, barrierDismissible: false, builder: (_) => AlertDialog(
            ...
            content: Column(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.check_circle, color: Colors.green, size: 60), const SizedBox(height: 16),
              const Text('Purchase Successful', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 8),
              Text('Royal Group Care ${widget.optionTitle}', style: TextStyle(fontSize: 13, color: Colors.grey[600])),
  ...
  Future<void> _pickDob() async {
    ...
    if (picked != null) setState(() => _memberDobController.text = '${picked.day.toString().padLeft(2, '0')}/${picked.month.toString().padLeft(2, '0')}/${picked.year}');
  }

  void _addMember() {
    if (_memberNameController.text.trim().isEmpty || _memberGender == null || _memberDobController.text.trim().isEmpty || _memberOccupationController.text.trim().isEmpty || _memberPhoneController.text.trim().isEmpty || _memberEmailController.text.trim().isEmpty || _nokNameController.text.trim().isEmpty || _nokPhoneController.text.trim().isEmpty || _nokRelationship == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill all fields including Next of Kin')));
      return;
    }
    setState(() {
      _coverMembers.add({
        'name': _memberNameController.text.trim(), 'gender': _memberGender!, 'dob': _memberDobController.text.trim(),
        'occupation': _memberOccupationController.text.trim(), 'phone': _memberPhoneController.text.trim(), 'email': _memberEmailController.text.trim(),
        'nokName': _nokNameController.text.trim(), 'nokPhone': _nokPhoneController.text.trim(), 'nokRelationship': _nokRelationship ?? '',
      });
      _memberNameController.clear(); _memberGender = null; _memberDobController.clear();
      _memberOccupationController.clear(); _memberPhoneController.clear(); _memberEmailController.clear();
      _nokNameController.clear(); _nokPhoneController.clear(); _nokRelationship = null;
    });
  }
  ...
  Future<void> _pickFile() async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) setState(() => _uploadedFileName = result.files.first.name);
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('File picker error: $e'), backgroundColor: Colors.red)); }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(elevation: 0,
        ...
        title: Text(_currentStep == 3 ? 'Royal Group Care summary' : 'Royal Group Care', style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontWeight: FontWeight.bold, fontSize: 16)), centerTitle: true),
      body: SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(), child: Column(children: [
        _buildStepIndicator(), const Divider(height: 1),
  ...
  Widget _buildStepIndicator() {
    final labels = ['Group & Contact Information', 'Leadership & Risk Details', 'Risk Information', 'Summary'];
    ...
    return Padding(padding: EdgeInsets.fromLTRB(16, 8, 16, 12), child: Column(children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('Step $stepNum of $totalSteps', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppTheme.primaryNavy)),
      ...
      const SizedBox(height: 8),
      Row(children: List.generate(totalSteps, (i) => Expanded(child: Container(height: 3, margin: EdgeInsets.only(right: i < totalSteps - 1 ? 4 : 0),
        decoration: BoxDecoration(color: i < stepNum ? AppTheme.primaryNavy : Colors.grey[300], borderRadius: BorderRadius.circular(2)))))),
    ]));
  }
  ...
  Widget _buildGroupInfoStep() {
    return Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _label('Name of Group *'), const SizedBox(height: 6),
      _tf('Enter your group name', _groupNameController, autofillHints: [AutofillHints.name]),
      const SizedBox(height: 16),
      _label('Email *'), const SizedBox(height: 6),
      _tf('Enter your email', _groupEmailController, keyboardType: TextInputType.emailAddress, autofillHints: [AutofillHints.email]),
      const SizedBox(height: 16),
      _label('Address *'), const SizedBox(height: 6),
      _tf('Enter your address', _groupAddressController, autofillHints: [AutofillHints.streetAddressLine1]),
      const SizedBox(height: 16),
      _label('Business Sector *'), const SizedBox(height: 6),
      _dd('Select your business sector', _selectedBusinessSector, _businessSectors, (v) => setState(() => _selectedBusinessSector = v)),
      const SizedBox(height: 16),
      _label('Website'), const SizedBox(height: 6),
      _tf('Enter your company website', _websiteController),
      const SizedBox(height: 40),
      _btn('Next', _groupNameController.text.trim().isNotEmpty && _groupEmailController.text.trim().isNotEmpty && _groupAddressController.text.trim().isNotEmpty && _selectedBusinessSector != null && _websiteController.text.trim().isNotEmpty
  ...
  Widget _buildLeadershipStep() {
    return Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 12),
      _label("Chairman's Name"), const SizedBox(height: 6), _tf('', _chairNameController, autofillHints: [AutofillHints.name]),
      const SizedBox(height: 12),
      _label("Chairman's Phone Number"), const SizedBox(height: 6), _tf('', _chairPhoneController, keyboardType: TextInputType.phone, autofillHints: [AutofillHints.telephoneNumber]),
      const SizedBox(height: 12),
      _label("Chairman's NIN"), const SizedBox(height: 6),
      Row(children: [
        Expanded(child: _tf('', _chairNinController, keyboardType: TextInputType.number, maxLength: 11)),
      ...
      const SizedBox(height: 12),
      _label("Secretary's Name"), const SizedBox(height: 6), _tf('', _secNameController, autofillHints: [AutofillHints.name]),
      const SizedBox(height: 12),
      _label("Secretary's Phone Number"), const SizedBox(height: 6), _tf('', _secPhoneController, keyboardType: TextInputType.phone, autofillHints: [AutofillHints.telephoneNumber]),
      const SizedBox(height: 12),
      _label("Secretary's NIN"), const SizedBox(height: 6),
      Row(children: [
        Expanded(child: _tf('', _secNinController, keyboardType: TextInputType.number, maxLength: 11)),
        const SizedBox(width: 8),
        SizedBox(width: 80, height: 48, child: ElevatedButton(
      ...
      const SizedBox(height: 40),
      _btn('Continue', _chairNameController.text.trim().isNotEmpty && _chairPhoneController.text.trim().isNotEmpty && _chairNinController.text.trim().isNotEmpty && _secNameController.text.trim().isNotEmpty && _secPhoneController.text.trim().isNotEmpty && _secNinController.text.trim().isNotEmpty
  ...
  Widget _sectionHeader(String title) {
    return Container(width: double.infinity, padding: EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8)),
      child: Row(children: [Expanded(child: Text(title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface))), Icon(Icons.remove, size: 18, color: Colors.grey)]));
  }
  ...
  Widget _buildMembersStep() {
    return Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        const Text('Click to add single members', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
        const SizedBox(width: 8),
        GestureDetector(onTap: _addMember, child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: AppTheme.primaryNavy, borderRadius: BorderRadius.circular(16)),
      ...
      const SizedBox(height: 16),
      _label('Name *'), const SizedBox(height: 6), _tf('enter full name', _memberNameController, autofillHints: [AutofillHints.name]),
      const SizedBox(height: 12),
      _label('Gender *'), const SizedBox(height: 6), _dd('select', _memberGender, _genders, (v) => setState(() => _memberGender = v)),
      const SizedBox(height: 12),
      ...
      const SizedBox(height: 12),
      _label('Email *'), const SizedBox(height: 6), _tf('email', _memberEmailController, keyboardType: TextInputType.emailAddress, autofillHints: [AutofillHints.email]),
      const SizedBox(height: 16),
      _sectionHeader('Next of Kin Information'),
      const SizedBox(height: 12),
      _label('Name *'), const SizedBox(height: 6), _tf('enter full name', _nokNameController, autofillHints: [AutofillHints.name]),
      const SizedBox(height: 12),
      _label('Phone Number *'), const SizedBox(height: 6), _tf('enter phone number', _nokPhoneController, keyboardType: TextInputType.phone, autofillHints: [AutofillHints.telephoneNumber]),
      const SizedBox(height: 12),
      ...
      if (_coverMembers.isNotEmpty) ...[
        const SizedBox(height: 8),
        SizedBox(width: double.infinity, child: OutlinedButton(onPressed: () => setState(() { if (_coverMembers.isNotEmpty) _coverMembers.removeLast(); }),
          style: OutlinedButton.styleFrom(foregroundColor: Colors.red, side: const BorderSide(color: Colors.red), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
        ...
        ...List.generate(_coverMembers.length, (i) {
          ...
          return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.grey[50], borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey[200]!)),
            child: Column(children: [_mRow('Name', m['name'] ?? ''), _mRow('Gender', m['gender'] ?? ''), _mRow('DOB', m['dob'] ?? ''), _mRow('Phone', m['phone'] ?? ''), _mRow('NOK', m['nokName'] ?? '')]));
        }),
      ],
      const SizedBox(height: 20),
      const Text('Click to upload bulk members', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
      const SizedBox(height: 8),
      Row(children: [
      ...
      const SizedBox(height: 8),
      GestureDetector(onTap: _pickFile, child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 24),
        decoration: BoxDecoration(border: Border.all(color: Colors.grey[300]!), borderRadius: BorderRadius.circular(8)),
        child: Column(children: [Icon(Icons.cloud_upload_outlined, size: 32, color: Colors.grey[400]), Text('Drop your files here', style: TextStyle(fontSize: 12, color: Colors.grey[500])), Text('Browse file from your phone', style: TextStyle(fontSize: 11, color: Colors.blue[400]))]))),
      if (_uploadedFileName != null) ...[
        const SizedBox(height: 8),
        Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(8)),
          child: Row(children: [const Icon(Icons.insert_drive_file, size: 16, color: Colors.grey), const SizedBox(width: 8), Expanded(child: Text(_uploadedFileName!, style: const TextStyle(fontSize: 12))), GestureDetector(onTap: () => setState(() => _uploadedFileName = null), child: const Icon(Icons.close, size: 16, color: Colors.red))])),
      ],
      const SizedBox(height: 20),
      ...
      SizedBox(height: 24),
      _btn('Submit', (_coverMembers.isNotEmpty || _uploadedFileName != null) && _consentChecked ? () => setState(() => _currentStep = 3) : null),
      SizedBox(height: 12),
      _outlineBtn('Back', () => setState(() => _currentStep = 1)),
    ]));
  }

  Widget _mRow(String l, String v) => Padding(padding: EdgeInsets.only(bottom: 4), child: Row(children: [
  ...
  Widget _buildSummaryStep() {
    return Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _sec('Payment Information', [_sRow('Product', 'Royal Group Care ${widget.optionTitle}'), _sRow('Price', widget.price), _sRow('Paystack Charges', _formatNaira(_getPaystackCharge())), _sRow('Total', _formatNaira(_getTotalAmount()))]),
      const SizedBox(height: 20),
      _sec('Group & Contact Information', [_sRow('Name of Group', _groupNameController.text), _sRow('Email', _groupEmailController.text), _sRow('Address', _groupAddressController.text), _sRow('Business Sector', _selectedBusinessSector ?? '-'), _sRow('Website', _websiteController.text)]),
      const SizedBox(height: 20),
      _sec('Leadership & Risk Details', [_sRow("Chairman's Name", _chairNameController.text), _sRow("Chairman's Phone", _chairPhoneController.text), _sRow("Chairman's NIN", _chairNinController.text), _sRow("Secretary's Name", _secNameController.text), _sRow("Secretary's Phone", _secPhoneController.text), _sRow("Secretary's NIN", _secNinController.text)]),
      if (_coverMembers.isNotEmpty) ...[
        const SizedBox(height: 20),
        _sec('Cover Members (${_coverMembers.length})', [
          ..._coverMembers.map((m) => Padding(padding: const EdgeInsets.only(bottom: 8), child: Column(children: [
            _sRow('Name', m['name'] ?? ''), _sRow('Phone', m['phone'] ?? ''), _sRow('NOK', m['nokName'] ?? ''), _sRow('Relationship', m['nokRelationship'] ?? ''),
            if (_coverMembers.indexOf(m) < _coverMembers.length - 1) Divider(color: Colors.grey[300]),
          ]))),
        ]),
      ],
  ...
  Widget _sec(String title, List<Widget> rows) => Container(width: double.infinity, padding: EdgeInsets.all(16),
    decoration: BoxDecoration(color: Colors.grey[50], borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey[200]!)),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)), SizedBox(height: 12), ...rows]));
  ...
  Widget _tf(String hint, TextEditingController c, {TextInputType? keyboardType, int maxLines = 1, int? maxLength, Widget? suffixIcon, List<String>? autofillHints}) {
    String? errorText;
    ...
    return TextField(
      ...
      decoration: InputDecoration(hintText: hint, hintStyle: TextStyle(color: Colors.grey[400], fontSize: 13), filled: true, fillColor: Theme.of(context).brightness == Brightness.dark ? const Color(0xFF1E1E1E) : Colors.white, counterText: '', suffixIcon: suffixIcon,
        errorText: errorText,
  ...
  Widget _dd(String hint, String? value, List<String> items, ValueChanged<String?> onChanged) => Container(
    decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey[300]!)),
    child: DropdownButtonFormField<String>(value: value, hint: Text(hint, style: TextStyle(color: Colors.grey[400], fontSize: 13)),
