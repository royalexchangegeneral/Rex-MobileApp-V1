...
class _VehicleInformationScreenState extends State<VehicleInformationScreen> {
  ...
  Future<void> _handleVerify() async {
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({'Intcode': 'Testcode', 'Password': 'royal1234', 'RegNo': regNo}),
      ).timeout(const Duration(seconds: 15));
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (data['status'] == 'Successful' && data['data'] != null && (data['data'] as List).isNotEmpty) {
          ...
          final hasRealData = (firstItem['vehicleMake']?.toString() ?? '').isNotEmpty &&
              firstItem['vehiclestatus']?.toString() != 'Non-Existent Registration';
          if (hasRealData) {
            setState(() { _isVerifying = false; _isVerified = true; _vehicleData = firstItem; });
            await Future.delayed(const Duration(milliseconds: 800));
            if (mounted) {
              Navigator.push(context, MaterialPageRoute(builder: (_) => PrivateMotorDetailsScreen(
                vehicleType: widget.vehicleType, price: widget.price, personalInfo: widget.personalInfo,
                vehicleData: _vehicleData!, isLoggedIn: widget.isLoggedIn, isAgent: widget.isAgent, agentCode: widget.agentCode)));
            }
          } else {
            setState(() { _isVerifying = false; _manualEntry = true; });
          ...
          setState(() { _isVerifying = false; _manualEntry = true; });
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(data['message']?.toString() ?? 'Vehicle not found. Enter details manually.'), backgroundColor: Colors.orange),
          );
        }
      } else {
        setState(() { _isVerifying = false; _manualEntry = true; });
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Server error. Enter details manually.'), backgroundColor: Colors.orange));
      }
    } catch (e) {
      if (mounted) {
        setState(() { _isVerifying = false; _manualEntry = true; });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: $e. Enter details manually.'), backgroundColor: Colors.orange));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        elevation: 0,
        leading: IconButton(icon: Icon(Icons.arrow_back, color: onSurface), onPressed: () => Navigator.pop(context)),
        title: Text(widget.vehicleType, style: TextStyle(color: onSurface, fontSize: 18, fontWeight: FontWeight.w600)),
        centerTitle: true,
      ),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Reg Number', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: onSurface)),
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: borderColor, width: 1.5)),
                      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: borderColor, width: 1.5)),
                  ...
                  if (_manualEntry && !_isVerified) ...[
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(child: Text('Vehicle not found. Enter details manually.', style: TextStyle(fontSize: 11, color: Colors.orange[700]))),
                      ]),
                    ),
                    const SizedBox(height: 16),
                    _buildField('VIN / Chassis Number *', 'enter VIN', _vinController),
                    ...
                    _buildField('Engine Number *', 'enter engine number', _engineController),
                        ...
                        onPressed: _selectedMake != null &&
                            ...
                            ? () {
                                final manualData = <String, dynamic>{
                                  ...
                                  'vehicleEngineno': _engineController.text.trim(),
                                ...
                                Navigator.push(context, MaterialPageRoute(builder: (_) => PrivateMotorDetailsScreen(
                                  vehicleType: widget.vehicleType, price: widget.price, personalInfo: widget.personalInfo,
                                  vehicleData: manualData, isLoggedIn: widget.isLoggedIn, isAgent: widget.isAgent, agentCode: widget.agentCode)));
                              }
                            : null,
                        style: ElevatedButton.styleFrom(
  ...
  Widget _buildField(String label, String hint, TextEditingController controller, {TextInputType? keyboardType}) {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 6),
        TextField(
          controller: controller, keyboardType: keyboardType, onChanged: (_) => setState(() {}),
          ...
          decoration: InputDecoration(
            ...
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: isDark ? Colors.grey[700]! : Colors.grey[300]!)),
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide(color: isDark ? Colors.grey[700]! : Colors.grey[300]!)),
            focusedBorder: const OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(8)), borderSide: BorderSide(color: AppTheme.primaryNavy, width: 2)),
