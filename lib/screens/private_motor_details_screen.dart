...
class PrivateMotorDetailsScreen extends StatelessWidget {
  ...
  // Paystack keys
  static const String _paystackPublicKey = 'pk_test_011f70659dfdf620a4950ef3390860c6f9ee6224';
  static const String _paystackSecretKey = 'sk_test_dd6287962b39d4040217583eb0c2abef0d1239b5';

  int _parsePriceToKobo() {
  ...
  Future<void> _initiatePayment(BuildContext context) async {
    ...
    final amountInKobo = _parsePriceToKobo();
    final ref = 'REX_${DateTime.now().millisecondsSinceEpoch}';
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({
          ...
          'reference': ref,
          'currency': 'NGN',
          'callback_url': 'https://rexinsure.com/payment-callback',
        }),
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == true) {
          ...
          final result = await Navigator.push<bool>(
            context,
            MaterialPageRoute(builder: (_) => _PaystackWebView(url: authUrl, callbackUrl: 'https://rexinsure.com/payment-callback')),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(data['message'] ?? 'Payment initialization failed'), backgroundColor: Colors.red),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('$vehicleType Details', style: const TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.w600)),
        centerTitle: true,
      ),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(flex: 3, child: Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.black87), textAlign: TextAlign.right)),
        ],
      ),
    );
  }
