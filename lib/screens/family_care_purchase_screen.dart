...
class FamilyCarePurchaseScreen extends StatefulWidget {
  ...
  final String price;
  const FamilyCarePurchaseScreen({super.key, required this.optionTitle, required this.price});
  @override
  State<FamilyCarePurchaseScreen> createState() => _FamilyCarePurchaseScreenState();
}

class _FamilyCarePurchaseScreenState extends State<FamilyCarePurchaseScreen> {
  int _currentStep = 0; // 0=NIN, 1=socioeconomic, 2=next of kin, 3=cover members
  ...
  Future<void> _verifyNin() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseData['status'] == 'success' && responseData['data']?['data']?['kyc'] != null && responseData['data']['data']['kyc']['firstname'] != null && (responseData['data']['data']['kyc']['firstname']?.toString() ?? '').isNotEmpty) {
  ...
  double _getBaseAmount() {
    ...
    final prices = matches.map((m) => double.tryParse(m.group(0)!.replaceAll(',', '')) ?? 0).toList();
    if (prices.length >= 2) {
      final parentPrice = prices[0];
      final childPrice = prices[1];
      final parentCount = _coverMembers.where((m) => m['category']?.toLowerCase() == 'parent' || m['category']?.toLowerCase() == 'spouse').length;
      final childCount = _coverMembers.where((m) => m['category']?.toLowerCase() == 'child').length;
  ...
  Future<void> _initiatePayment() async {
    ...
    final priceMatches = RegExp(r'[\d,]+\.?\d*').allMatches(widget.price).toList();
    final parentPremium = priceMatches.isNotEmpty ? (double.tryParse(priceMatches[0].group(0)!.replaceAll(',', '')) ?? 0).toInt() : premiumAmount.toInt();
    final childPremium = priceMatches.length >= 2 ? (double.tryParse(priceMatches[1].group(0)!.replaceAll(',', '')) ?? 0).toInt() : 0;
    ...
    try {
      final result = await PaymentService.initiatePurchase(
        ...
        extraFields: {
          ...
          'pdsumassured': 0,
          'medisumassured': 0,
          'members': _coverMembers.map((m) => {'names': m['name'] ?? '', 'gender': m['gender'] ?? '', 'dob': m['dob'] ?? '', 'relationship': m['category'] ?? ''}).toList(),
        },
      );
        ...
        if (res != null && res.success && mounted) {
          showDialog(context: context, barrierDismissible: false, builder: (_) => AlertDialog(
            ...
            content: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text('Royal Family Care ${widget.optionTitle}', style: TextStyle(fontSize: 13, color: Colors.grey[600])),
  ...
  Future<void> _pickDob() async {
    ...
    if (picked != null) {
      setState(() => _memberDobController.text = '${picked.day.toString().padLeft(2, '0')}/${picked.month.toString().padLeft(2, '0')}/${picked.year}');
    }
  }

  void _addMember() {
    if (_memberNameController.text.trim().isEmpty || _memberGender == null || _memberDobController.text.trim().isEmpty || _memberCategory == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill all member fields')));
      return;
    }
    setState(() {
      _coverMembers.add({'name': _memberNameController.text.trim(), 'gender': _memberGender!, 'dob': _memberDobController.text.trim(), 'category': _memberCategory!});
  ...
  Future<void> _pickFile() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('File picker error: $e. Please restart the app after a full rebuild.'), backgroundColor: Colors.red));
      }
    }
  }
  ...
  Future<void> _downloadSampleFile() async {
    try {
      ...
      await file.writeAsString('name,gender,dob,category\nkamoru isa,male,1994-04-20,parent\nkil isa,female,1990-04-23,Parent\nJayo,Male,1991-05-30,child\n');
      if (mounted) {
        final box = context.findRenderObject() as RenderBox?;
        final origin = box != null ? box.localToGlobal(Offset.zero) & box.size : const Rect.fromLTWH(0, 0, 100, 100);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Sample file saved to: ${file.path}'), backgroundColor: Colors.green));
        await Share.shareXFiles([XFile(file.path)], subject: 'Family Information Sample', sharePositionOrigin: origin);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(_currentStep == 4 ? 'Royal Family Care' : 'Royal Family Care',
  ...
  Widget _buildStepIndicator() {
    final labels = ['Mode of Identification', 'Socioeconomic Information', 'Next of Kin / Beneficiary Info', 'Add/Upload Cover Members', 'Summary'];
    final totalSteps = _currentStep == 4 ? 3 : 4;
    final stepNum = _currentStep == 4 ? 3 : _currentStep + 1;
    final label = _currentStep < labels.length ? labels[_currentStep] : 'Summary';
    return Padding(
      ...
      child: Column(children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          Flexible(child: Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Theme.of(context).colorScheme.onSurface), textAlign: TextAlign.right)),
        ]),
        const SizedBox(height: 8),
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(hintText: 'Enter your 11-digit NIN', hintStyle: TextStyle(color: Colors.grey[400], fontSize: 13), filled: true, fillColor: Theme.of(context).brightness == Brightness.dark ? const Color(0xFF1E1E1E) : Colors.white, counterText: '',
        ...
        if (!_ninFailed && !_ninVerified) ...[
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: _ninController.text.trim().length == 11 && !_isVerifying ? _verifyNin : null,
  ...
  Widget _buildSocioeconomicStep() {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 40),
        SizedBox(width: double.infinity, child: ElevatedButton(
          onPressed: _occupationController.text.trim().isNotEmpty && _selectedQualification != null && _annualIncomeController.text.trim().isNotEmpty && _emailController.text.trim().isNotEmpty
  ...
  Widget _buildNextOfKinStep() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 16),
        _label('Relationship'), const SizedBox(height: 6),
        _dropdown('select the type of relationship', _selectedRelationship, _relationships, (v) => setState(() => _selectedRelationship = v)),
        const SizedBox(height: 40),
        SizedBox(width: double.infinity, child: ElevatedButton(
          onPressed: _nokNameController.text.trim().isNotEmpty && _nokAddressController.text.trim().isNotEmpty && _nokPhoneController.text.trim().isNotEmpty && _selectedRelationship != null
  ...
  Widget _buildCoverMembersStep() {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // Add single members
        Row(children: [
          Text('Click to add single members', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Theme.of(context).colorScheme.onSurface)),
          const SizedBox(width: 8),
          GestureDetector(
            onTap: _addMember,
            child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: AppTheme.primaryNavy, borderRadius: BorderRadius.circular(16)),
        ...
        _dropdown('select', _memberGender, _genders, (v) => setState(() => _memberGender = v)),
        const SizedBox(height: 12),
        _label('Date of Birth'), const SizedBox(height: 6),
        GestureDetector(
          onTap: _pickDob,
          child: AbsorbPointer(child: _textField('dd/mm/yyyy', _memberDobController, suffixIcon: Icon(Icons.calendar_today, size: 18, color: Colors.grey))),
        ),
        const SizedBox(height: 12),
        _label('Category'), const SizedBox(height: 6),
        _dropdown('parent', _memberCategory, _categories, (v) => setState(() => _memberCategory = v)),
        const SizedBox(height: 12),
        SizedBox(width: double.infinity, child: ElevatedButton(
          onPressed: _addMember,
          style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryNavy, foregroundColor: Colors.white, padding: EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
        ...
        if (_coverMembers.isNotEmpty) ...[
          SizedBox(height: 20),
          Row(children: [
            Text('Cover Members', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
            const Spacer(),
            GestureDetector(onTap: () => setState(() => _coverMembers.clear()), child: const Icon(Icons.cancel, color: Colors.grey, size: 20)),
          ]),
          const SizedBox(height: 8),
            ...
            return Container(
              ...
              child: Column(children: [
                ...
                Row(children: [
                  ...
                  GestureDetector(onTap: () => setState(() => _coverMembers.removeAt(i)), child: const Icon(Icons.delete_outline, color: Colors.red, size: 18)),
                ]),
              ]),
            );
          }),
        ...
        const SizedBox(height: 24),
        // Bulk upload
        Text('Click to upload bulk members', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Theme.of(context).colorScheme.onSurface)),
        const SizedBox(height: 8),
        OutlinedButton.icon(
          onPressed: _downloadSampleFile,
          icon: Icon(Icons.download, size: 16),
          label: const Text('Download sample file', style: TextStyle(fontSize: 12)),
          style: OutlinedButton.styleFrom(foregroundColor: AppTheme.primaryNavy, side: const BorderSide(color: AppTheme.primaryNavy), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
        ),
        const SizedBox(height: 12),
        const Text('Upload a file (Upload an Excel file max 2 MB)', style: TextStyle(fontSize: 11, color: Colors.grey)),
        const SizedBox(height: 8),
        GestureDetector(
          onTap: _pickFile,
          child: Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 4),
              Text('Drop your files here', style: TextStyle(fontSize: 12, color: Colors.grey[500])),
              Text('Browse file from your phone', style: TextStyle(fontSize: 11, color: Colors.blue[400])),
            ]),
          ),
        ),
        if (_uploadedFileName != null) ...[
          ...
          Container(
            ...
            child: Row(children: [
              ...
              SizedBox(width: 8),
              Expanded(child: Text(_uploadedFileName!, style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface))),
              GestureDetector(onTap: () => setState(() => _uploadedFileName = null), child: const Icon(Icons.close, size: 16, color: Colors.red)),
            ]),
          ),
        ],
        const SizedBox(height: 20),
        ...
        const SizedBox(height: 24),
        SizedBox(width: double.infinity, child: ElevatedButton(
          onPressed: (_coverMembers.isNotEmpty || _uploadedFileName != null) && _consentChecked ? () => setState(() => _currentStep = 4) : null,
          style: ElevatedButton.styleFrom(backgroundColor: AppTheme.primaryNavy, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), disabledBackgroundColor: Colors.grey[300]),
  ...
  Widget _memberRow(String label, String value) {
    return Padding(
      ...
      child: Row(children: [
        Expanded(flex: 2, child: Text(label, style: TextStyle(fontSize: 11, color: Colors.grey[600]))),
        SizedBox(width: 8),
        Expanded(flex: 3, child: Text(value, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Theme.of(context).colorScheme.onSurface), textAlign: TextAlign.right)),
      ]),
    );
  }
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _summarySection('Leadership & Risk Details', [
          ...
          _summaryRow('Tax Identification No', _tinController.text.isNotEmpty ? _tinController.text : '-'),
        ...
        if (_coverMembers.isNotEmpty) ...[
          ...
          _summarySection('Added Members (${_coverMembers.length})', [
            ..._coverMembers.map((m) => Padding(
              ...
              child: Row(children: [
                Expanded(child: Text('${m['name']} (${m['category']})', style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface))),
                Text('${m['gender']} - ${m['dob']}', style: TextStyle(fontSize: 11, color: Colors.grey[600])),
              ]),
            )),
          ]),
        ],
  ...
  Widget _label(String text) => Text(text, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Theme.of(context).colorScheme.onSurface));

  Widget _textField(String hint, TextEditingController controller, {TextInputType? keyboardType, int maxLines = 1, int? maxLength, Widget? suffixIcon, List<String>? autofillHints}) {
    String? errorText;
    ...
    return TextField(
      controller: controller, keyboardType: keyboardType, maxLines: maxLines, maxLength: maxLength,
      onChanged: (_) => setState(() {}),
