...
class ShopProtectionPurchaseScreen extends StatefulWidget {
  ...
  const ShopProtectionPurchaseScreen({super.key, required this.optionTitle, required this.price, this.productName = 'Shop Protection Plan'});
  @override
  State<ShopProtectionPurchaseScreen> createState() => _ShopProtectionPurchaseScreenState();
}

class _ShopProtectionPurchaseScreenState extends State<ShopProtectionPurchaseScreen> {
  ...
  final List<String> _shopDescriptions = const ['Detached', 'Semi-Detached', 'Flat', 'Bungalow', 'Duplex', 'Shop', 'Warehouse', 'Others'];
  final List<String> _categoryOptions = const ['Furniture', 'Electronics', 'Clothing', 'Food Items', 'Building Materials', 'Machinery', 'Stationery', 'Others'];
  ...
  Future<void> _verifyNin() async {
    ...
    try {
      final response = await http.post(Uri.parse('https://eportaltest.rexinsure.com/api/verify/nin'),
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (data['status'] == 'success' && data['data']?['data']?['kyc'] != null && data['data']['data']['kyc']['firstname'] != null && (data['data']['data']['kyc']['firstname']?.toString() ?? '').isNotEmpty) {
          ...
          setState(() {
            _firstName = kyc['firstname']?.toString() ?? ''; _lastName = kyc['surname']?.toString() ?? '';
            _email = kyc['email']?.toString() ?? ''; _emailController.text = _email;
  ...
  String _formatNaira(double a) => '₦${a.toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+\.)'), (m) => '${m[1]},')}';

  Future<void> _initiatePayment() async {
    final email = _email.isNotEmpty ? _email : 'customer@rexinsure.com';
    final premiumAmount = _getBaseAmount();
    final productCode = widget.productName.toLowerCase().contains('home') ? 'HP' : 'SH';
    ...
    try {
      final result = await PaymentService.initiatePurchase(
        ...
        extraFields: {
          ...
          'items': _itemCategories.map((item) => {'name': item['category'] ?? '', 'amount': int.tryParse(item['amount']?.replaceAll(',', '') ?? '0') ?? 0}).toList(),
        },
      );
  ...
  void _addCategory() {
    if (_tempCategory == null || _tempAmountController.text.trim().isEmpty) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select category and enter amount'))); return; }
    setState(() { _itemCategories.add({'category': _tempCategory!, 'amount': _tempAmountController.text.trim()}); _tempCategory = null; _tempAmountController.clear(); });
  }
  ...
  Widget _stepIndicator() {
    ...
    return Padding(padding: const EdgeInsets.fromLTRB(16, 8, 16, 12), child: Column(children: [
      ...
      const SizedBox(height: 8),
      Row(children: List.generate(4, (i) => Expanded(child: Container(height: 3, margin: EdgeInsets.only(right: i < 3 ? 4 : 0),
  ...
  Widget _ninStep() => Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    if (!_ninFailed && !_ninVerified) ...[
      ...
      _btn('Continue', _ninController.text.trim().length == 11 && !_isVerifying ? _verifyNin : null, loading: _isVerifying),
    ],
    if (_ninVerified) ...[
      ...
      _tf('first name', _manualFirstNameController, autofillHints: [AutofillHints.givenName]),
      const SizedBox(height: 12),
      _label('Last Name *'), const SizedBox(height: 6),
      _tf('last name', _manualLastNameController, autofillHints: [AutofillHints.familyName]),
      const SizedBox(height: 12),
      _label('Email *'), const SizedBox(height: 6),
      _tf('email', _manualEmailController, keyboardType: TextInputType.emailAddress, autofillHints: [AutofillHints.email]),
      const SizedBox(height: 12),
      _label('Phone Number *'), const SizedBox(height: 6),
      _tf('phone number', _manualPhoneController, keyboardType: TextInputType.phone, autofillHints: [AutofillHints.telephoneNumber]),
      const SizedBox(height: 12),
      _label('Address'), const SizedBox(height: 6),
      _tf('address', _manualAddressController, maxLines: 2, autofillHints: [AutofillHints.streetAddressLine1]),
      const SizedBox(height: 12),
      ...
      _btn('Continue', _manualFirstNameController.text.trim().isNotEmpty &&
    ...
    if (_ninFailed) ...[
      ...
      const SizedBox(height: 16),
      _label('First Name *'), const SizedBox(height: 6),
      _tf('enter first name', _manualFirstNameController, autofillHints: [AutofillHints.givenName]),
      const SizedBox(height: 12),
      _label('Last Name *'), const SizedBox(height: 6),
      _tf('enter last name', _manualLastNameController, autofillHints: [AutofillHints.familyName]),
      const SizedBox(height: 12),
      _label('Email *'), const SizedBox(height: 6),
      _tf('enter email', _manualEmailController, keyboardType: TextInputType.emailAddress, autofillHints: [AutofillHints.email]),
      const SizedBox(height: 12),
      _label('Phone Number *'), const SizedBox(height: 6),
      _tf('enter phone number', _manualPhoneController, keyboardType: TextInputType.phone, autofillHints: [AutofillHints.telephoneNumber]),
      const SizedBox(height: 12),
      _label('Address'), const SizedBox(height: 6),
      _tf('enter address', _manualAddressController, maxLines: 2, autofillHints: [AutofillHints.streetAddressLine1]),
      const SizedBox(height: 12),
      _label('State'), const SizedBox(height: 6),
      SearchableDropdown(
        hint: 'select state',
        ...
        return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(12),
          ...
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('ITEMS CATEGORY ${i + 1}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1)),
  ...
  Widget _summaryStep() => Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    _sec('Payment Information', [_sRow('Product', '${widget.productName} ${widget.optionTitle}'), _sRow('Price', widget.price), _sRow('Paystack Charges', _formatNaira(_getPaystackCharge())), _sRow('Total', _formatNaira(_getTotalAmount()))]),
    const SizedBox(height: 20),
