...
class PolicyDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Policy Details', style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontWeight: FontWeight.bold, fontSize: 18)),
        centerTitle: true,
        actions: const [],
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(policyClass, style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.bold)),
            ...
            Row(
              children: [
                Expanded(child: _buildQuickAction(Icons.refresh, 'Renew Policy', const Color(0xFFE8F4FD), const Color(0xFF4A90D9), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => PolicyRenewalScreen(policyType: policyType, policyNumber: policyNumber, premium: policyData?['premium']?.toString() ?? policyData?['Premium']?.toString() ?? '0', isAgentFlow: isAgentFlow, policyData: policyData))))),
            ...
            Text('Policy Details', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
            ...
            // Documents
            Text('Documents', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
            const SizedBox(height: 12),
            ...
            Container(
              ...
              child: Column(
                children: [
                  _buildDocumentItem(context, 'Policy Document', Icons.picture_as_pdf, Colors.red),
                ],
              ),
            ),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text('Talk to a Support Agent', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
                    ],
                  ),
                  SizedBox(height: 10),
                  Row(
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Rex Support', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                onPressed: () {
                  ...
                  final certUrl = 'https://eportaltest.rexinsure.com/$certType?policy=$policyNumber';
                  Navigator.push(context, MaterialPageRoute(builder: (_) => _CertificateViewScreen(url: certUrl, policyNumber: policyNumber)));
                },
      ...
      floatingActionButton: isAgentFlow ? null : Transform.translate(
    ...
    return GestureDetector(
      ...
      child: Column(
        children: [
          ...
          SizedBox(height: 6),
          Text(label, style: TextStyle(fontSize: 10, color: Colors.black87, fontWeight: FontWeight.w500), textAlign: TextAlign.center),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Flexible(child: Text(value.isNotEmpty ? value : 'N/A', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.black87), textAlign: TextAlign.right)),
        ],
      ),
    );
  }

  Widget _buildDocumentItem(BuildContext context, String title, IconData icon, Color iconColor) {
    return GestureDetector(
      ...
      child: Row(
        children: [
          ...
          SizedBox(width: 10),
          Expanded(child: Text(title, style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.onSurface))),
  ...
  String _getCertType() {
    final pClass = (policyData?['policyClass'] ?? policyData?['ProductClass'] ?? policyData?['ProductCover'] ?? policyType).toString().toLowerCase();
    ...
    if (pClass.contains('personal care') || pClass.contains('pc')) return 'pccert';
    if (pClass.contains('royal auto') || pClass.contains('bronze') || pClass.contains('silver')) return 'royalautocert';
  ...
  Future<void> _downloadPolicyDocument(BuildContext ctx) async {
    try {
      ...
      pdf.addPage(pw.Page(
        ...
        build: (pw.Context pdfCtx) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
          pw.Text('Policy Document', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 20),
          pw.Text('Policy Number: $policyNumber', style: const pw.TextStyle(fontSize: 14)),
          pw.Text('Policy Type: $policyType', style: const pw.TextStyle(fontSize: 14)),
          pw.SizedBox(height: 16),
          _pdfRow('Insured', data['Insured']?.toString() ?? '-'),
          _pdfRow('Product Class', data['policyClass']?.toString() ?? data['ProductClass']?.toString() ?? '-'),
          _pdfRow('Premium', 'N${data['Premium']?.toString() ?? data['premium']?.toString() ?? '0'}'),
          _pdfRow('Sum Insured', 'N${data['SumAssured']?.toString() ?? data['sumInsured']?.toString() ?? '0'}'),
          _pdfRow('Start Date', data['StartDate']?.toString() ?? data['startDate']?.toString() ?? '-'),
          _pdfRow('End Date', data['EndDate']?.toString() ?? data['endDate']?.toString() ?? '-'),
          _pdfRow('Status', data['status']?.toString() ?? '-'),
        ]),
      ));
      final dir = await getApplicationDocumentsDirectory();
      final file = File('${dir.path}/Policy_${policyNumber.replaceAll('/', '_')}.pdf');
      await file.writeAsBytes(await pdf.save());

      final box = ctx.findRenderObject() as RenderBox?;
      final origin = box != null ? box.localToGlobal(Offset.zero) & box.size : const Rect.fromLTWH(0, 0, 100, 100);
      await Share.shareXFiles([XFile(file.path)], subject: 'Policy Document - $policyNumber', sharePositionOrigin: origin);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Certificate - ${widget.policyNumber}', style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontSize: 14, fontWeight: FontWeight.w600)),
        centerTitle: true,
      ),
      body: Stack(children: [
