...
class PolicyProvider with ChangeNotifier {
  ...
  Future<void> fetchPolicies(BuildContext context) async {
    ...
    try {
      ...
      final phone = userData?['MobileNo']?.toString() ?? userData?['Phone']?.toString() ?? userData?['PhoneNo']?.toString() ?? '';
      ...
      print('=== GET CUSTOMER POLICY REQUEST ===');
      ...
      print('=== GET CUSTOMER POLICY RESPONSE: ${r.statusCode} ===');
            ...
            if (details != null) {
              ...
              if (details['Claim'] is List) {
                for (final c in details['Claim']) {
                  if (c['ClaimID'] != null) allClaims.add(Map<String, dynamic>.from(c));
                }
              }
            }
          }
    ...
    } catch (e) { print('Fetch policies error: $e'); }
